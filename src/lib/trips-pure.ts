/**
 * Edge-runtime-safe slice of the trip-clustering helpers.
 *
 * `src/lib/trips.ts` is the canonical entry point used by Server
 * Components and Route Handlers; it adds catalogue I/O via
 * `node:fs/promises`. That `node:fs` import bans it from the Edge
 * runtime even when only the pure helpers are needed (Webpack
 * still walks the module graph and emits an unresolvable
 * `node:fs` external).
 *
 * This file factors out everything that does not touch the
 * filesystem: types, slug helpers, and the `clusterTrips`
 * algorithm. Edge routes (e.g. the per-trip `opengraph-image`
 * route) can import from here AND statically import the catalogue
 * JSON without pulling Node-only modules into the Edge bundle.
 *
 * `trips.ts` re-exports `clusterTrips`, `Trip`, and `TripPhoto`
 * from this module so the existing public API does not change.
 */
...
/**
 * Pure clustering helper, exported for unit tests AND for Edge
 * routes that statically import the catalogue JSON. Takes raw
 * catalogue entries (already filtered to the ones with country +
 * date + GPS) and emits a list of Trips, applying:
 *
...
/**
 * Build the unique list of cities (in catalogue order) for a trip.
 * Used by the per-trip OG card to list where the trip went without
 * loading the photos themselves.
 */
export function tripCities(trip: Trip): string[] {
