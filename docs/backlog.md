...
- [ ] **`/my-story` page** *(Eduard)* — **benchmark complete; recommendation: SHIP shell, with tone constraints.** See `docs/audience-benchmark.md`. Constraints: chronological + dated, factual not heroic, 600–1200 words, "I worked on / learned" not "I led / revolutionised", cross-link `/work` and `/personal` rather than duplicate. Eduard writes prose; PO can do the layout shell next.
...
- [x] ~~Branch protection rules *(Architect)*~~ — runbook at `docs/branch-protection-setup.md`; PO applies the gh-CLI / web-UI commands once the PR is merged.
